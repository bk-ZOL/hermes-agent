...
class AIAgent:
    ...
    def generate_recap(self, messages: list = None) -> Optional[str]:
        """Generate and store an end-of-session recap summary.

        Called automatically on exit, /reset, and /resume (for the old session).
        Returns the recap text, or None if skipped (too short, API error, etc.).
        """
        _MIN_USER_TURNS = 3
        messages = messages or self._session_messages
        ...
        try:
            serialized = self._serialize_messages_for_recap(messages)
            if len(serialized) < 100:
                return None

            prompt = (
                "Summarize this conversation session in 2-4 concise paragraphs "
                "for the user's next session. Focus on:\n"
                "1. What was accomplished (files changed, commands run, outcomes)\n"
                "2. Key decisions and why they were made\n"
                "3. Unfinished or pending work\n\n"
                "Be specific with file paths, values, and concrete results. "
                "Write as a briefing for the next session, not as instructions.\n\n"
                "CONVERSATION:\n" + serialized
            )
            ...
            self._session_db.set_session_recap(self.session_id, recap)
            logger.info("Session recap generated and stored for %s", self.session_id)
            return recap
        except Exception as e:
            logger.debug("Failed to generate session recap: %s", e)
            return None

    def _serialize_messages_for_recap(self, messages: List[Dict[str, Any]]) -> str:
        """Serialize messages into a compact text format for recap generation."""
        _MAX = 800
        parts = []
        for msg in messages:
            role = msg.get("role", "")
