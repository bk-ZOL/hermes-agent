...
class SessionDB:
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 7:
                # v7: add recap column to sessions
                try:
                    cursor.execute("ALTER TABLE sessions ADD COLUMN recap TEXT")
                except sqlite3.OperationalError:
                    pass
    ...
    def set_session_recap(self, session_id: str, recap: str) -> None:
        """Store an end-of-session recap summary."""
        def _do(conn):
            conn.execute(
                "UPDATE sessions SET recap = ? WHERE id = ?",
                (recap, session_id),
            )
        self._execute_write(_do)

    def get_session_recap(self, session_id: str) -> Optional[str]:
        """Return the stored recap for a session, or None."""
        with self._lock:
            cursor = self._conn.execute(
                "SELECT recap FROM sessions WHERE id = ?", (session_id,)
            )
            row = cursor.fetchone()
        if not row:
            return None
        val = row["recap"] if isinstance(row, sqlite3.Row) else row[0]
